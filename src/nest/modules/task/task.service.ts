...
export class TaskService {
  ...
  private async loadInstanceMap(userId: number, taskIds: bigint[]) {
    if (!taskIds.length) return new Map<string, Awaited<ReturnType<typeof this.prisma.client.taskInstance.findMany>>[number]>();
  ...
  async listHallTasks(
    ...
    const page = Math.max(
      1,
      Number(query.curPage || query.page || 1) || 1,
    );
    ...
    const list = rows.map((row) => {
      ...
      return this.mapTask(
        row,
        this.buildUserProgress(inst, row.targetCount),
      );
    });
  ...
  async acceptTask(taskId: number, userId: number) {
    ...
    if (task.acceptValidHours && task.acceptValidHours > 0) {
      expiredAt = new Date(
        Date.now() + task.acceptValidHours * 60 * 60 * 1000,
      );
  ...
  async submitTask(
    ...
    if (hasChildren && subTaskIdRaw != null) {
      ...
      targetCount =
        children.length > 0 ? children.length : task.targetCount;
  ...
  async listRewardClaims() {
    ...
    const nameMap = new Map(
      users.map((u) => [
        Number(u.id),
        u.accountAlias || u.name || String(u.id),
      ]),
    );
  ...
  async getTaskStatistics(taskId: number) {
    ...
    const durations = instances
      ...
      .map(
        (i) =>
          (i.completedAt!.getTime() - i.acceptedAt.getTime()) /
          (1000 * 60 * 60),
      );
  ...
  async getUserStatistics(query: { limit?: number }) {
    ...
    const topUserIds = [...acceptMap.entries()]
      .sort((a, b) => (completeMap.get(b[0]) || 0) - (completeMap.get(a[0]) || 0))
      .slice(0, limit)
      .map(([uid]) => uid);
