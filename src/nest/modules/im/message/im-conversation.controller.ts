...
export class ImConversationController {
  ...
  async list(
    ...
    const data = await this.convService.listConversations(
      uid,
      Number(curPage),
      Number(pageSize)
    )
  ...
  async messages(
    ...
    const uid = String(req.auth?.uid ?? '')
    const data = await this.convService.listMessages(
      id,
      uid,
      beforeSeq ? Number(beforeSeq) : undefined,
      Number(limit)
    )
    return { status: true, msg: 'ok', data: serializeBigInt(data) }
  }
  ...
  async sync(
    ...
    const uid = String(req.auth?.uid ?? '')
    const data = await this.convService.syncMessages(
      id,
      uid,
      Number(afterSeq),
      Number(limit)
