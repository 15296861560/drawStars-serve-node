...
export class ImAdminController {
  ...
  async reports(@Query("status") status?: string, @Query("curPage") curPage = "1", @Query("pageSize") pageSize = "20") {
    const where: Record<string, unknown> = {};
    if (status) where.status = status;
    const [total, rows] = await Promise.all([
      prisma.imReport.count({ where }),
      prisma.imReport.findMany({ where, orderBy: { createdAt: "desc" }, skip: (Number(curPage) - 1) * Number(pageSize), take: Number(pageSize) }),
    ]);
    return { status: true, msg: "ok", data: serializeBigInt({ total, list: rows, curPage: Number(curPage), pageSize: Number(pageSize) }) };
  }

  @Put("reports/:id")
  @RequirePermissions("chat:moderation:operate")
  async handleReport(@Req() req: any, @Param("id") id: string, @Body() body: { status: string; remark?: string }) {
    const uid = String(req.auth?.uid ?? "0");
    const row = await prisma.imReport.update({
      where: { id: BigInt(id) },
      data: { status: body.status, remark: body.remark, handledBy: BigInt(uid), updatedAt: BigInt(Date.now()) },
    });
  ...
  async searchMessages(
    ...
    if (startTime || endTime) {
      where.serverTime = {};
      if (startTime) (where.serverTime as { gte: bigint }).gte = BigInt(startTime);
    ...
    const [total, rows] = await Promise.all([
      ...
      prisma.imMessage.findMany({ where, orderBy: { serverTime: "desc" }, skip: (Number(curPage) - 1) * Number(pageSize), take: Number(pageSize) }),
    ]);
    return { status: true, msg: "ok", data: serializeBigInt({ total, list: rows, curPage: Number(curPage), pageSize: Number(pageSize) }) };
  }
  ...
  async banRoom(@Param("roomId") roomId: string, @Body() body: { banned: boolean }) {
    ...
    const updated = await prisma.imRoom.update({
      where: { id: room.id },
      data: { status: body.banned ? RoomStatus.BANNED : RoomStatus.ACTIVE, updatedAt: BigInt(Date.now()) },
    });
  ...
  @RequirePermissions("system:chat:operate")
  async setWeight(@Param("roomId") roomId: string, @Body() body: { manualWeight?: number; official?: boolean }) {
    const room = await prisma.imRoom.findUnique({ where: { roomId } });
    if (!room) return { status: false, msg: "房间不存在", data: null };
  ...
  @Post("categories")
  @RequirePermissions("system:chat:operate")
  async createCategory(@Body() body: { code: string; name: string; sort?: number }) {
    const row = await prisma.imCategory.create({
      data: { code: body.code, name: body.name, sort: body.sort || 0, status: "ACTIVE" },
    });
    return { status: true, msg: "ok", data: serializeBigInt(row) };
  }
  ...
  async listSensitiveWords() {
    const rows = await prisma.imSensitiveWord.findMany({ orderBy: { createdAt: "desc" } });
    return { status: true, msg: "ok", data: serializeBigInt(rows) };
  }
  ...
  async addSensitiveWord(@Body() body: { word: string; action?: string }) {
    const row = await prisma.imSensitiveWord.upsert({
      ...
      create: { word: body.word, action: body.action || "BLOCK", createdAt: BigInt(Date.now()) },
