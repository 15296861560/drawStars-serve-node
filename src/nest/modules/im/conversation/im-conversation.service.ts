...
export class ImConversationService {
  ...
  async upsertC2C(userA: string, userB: string): Promise<bigint> {
    if (userA === userB) throw new ImException(ImErrorCode.NO_PERMISSION, "不能与自己单聊");
  ...
  async listConversations(uid: string, curPage = 1, pageSize = 50) {
    ...
    const lastMsgs = lastMsgIds.length
      ? await prisma.imMessage.findMany({ where: { msgId: { in: lastMsgIds } } })
      : [];
  ...
  /** 历史（beforeSeq 倒序分页） */
  async listMessages(conversationId: string, uid: string, beforeSeq?: number, limit = 20) {
  ...
  /** 补洞（afterSeq 升序） */
  async syncMessages(conversationId: string, uid: string, afterSeq: number, limit = 50) {
    limit = Math.min(50, Math.max(1, limit));
    const convId = BigInt(conversationId);
    await this.assertMember(convId, uid);
    const rows = await prisma.imMessage.findMany({
      where: { conversationId: convId, seq: { gt: Number(afterSeq) }, status: { in: ["NORMAL", "RECALLED"] } },
      orderBy: { seq: "asc" },
      take: limit,
    });
  ...
  async pin(conversationId: string, uid: string, pinned: boolean) {
    ...
    if (pinned) {
      ...
      if (pinnedCount >= IM_DEFAULTS.maxPinned) {
        throw new ImException(ImErrorCode.NO_PERMISSION, `置顶不超过 ${IM_DEFAULTS.maxPinned} 个`);
      }
    }
    const cu = await prisma.imConversationUser.findUnique({
      where: { conversationId_userId: { conversationId: convId, userId } },
  ...
  async deleteConversation(conversationId: string, uid: string) {
    ...
    const updated = await prisma.imConversationUser.update({
      where: { id: cu.id },
      data: { deletedAt: BigInt(Date.now()), unreadCount: 0, mentionUnread: 0, updatedAt: BigInt(Date.now()) },
    });
    return serializeBigInt(updated);
  }
  ...
  async assertMember(conversationId: bigint, uid: string | number) {
    ...
    if (!cu || cu.deletedAt) throw new ImException(ImErrorCode.NO_PERMISSION, "非会话成员");
    return cu;
  }
}
