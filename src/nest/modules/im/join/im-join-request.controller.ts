...
export class ImJoinRequestController {
  ...
  @Get()
  async list(@Req() req: any, @Query("scope") scope: "mine" | "manage" = "mine") {
  ...
  @Post()
  async create(@Req() req: any, @Body() body: { targetType: string; targetId: string; remark?: string }) {
    const uid = String(req.auth?.uid ?? "");
    const data = await this.reqService.create(uid, body.targetType, body.targetId, body.remark);
    return { status: true, msg: "ok", data: serializeBigInt(data) };
  }
