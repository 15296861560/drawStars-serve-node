...
export class ProfileController {
  ...
  @Get("queryUserInfo")
  async queryUserInfo(
    @Req() req: { headers: Record<string, unknown>; query: Record<string, unknown>; cookies?: Record<string, string>; auth?: { uid?: string } },
    @Query("id") id?: string,
  ) {
  ...
  uploadAvatar(
    ...
    form.parse(request, async (err, fields, files) => {
      if (err) {
        response.status(500).json({ status: false, msg: String(err), data: null });
        return;
      }
      try {
        ...
        if (!userId) {
          response.status(400).json({ status: false, msg: "缺少用户 id", data: null });
          return;
        }
        const orgName = fields.filename?.[0] as string;
        const oldpath = files.file?.[0]?.filepath;
        if (!oldpath) {
          response.status(400).json({ status: false, msg: "缺少文件", data: null });
          return;
        }
  ...
  @Post("getCaptcha")
  async getCaptcha(
    @Body() body: { account: string; type?: string },
  ) {
    try {
      if (!body.account) {
  ...
  @Post("verifyCaptcha")
  async verifyCaptcha(
    @Body() body: { account?: string; phone?: string; captcha: string; type?: string },
  ) {
