...
export class FillService {
  ...
  async submitResponse(
    ...
    const minDuration = await this.assertAntiCheat(
      survey.id,
      pub,
      {
        userId,
      ...
      for (const a of answers) {
        ...
        await tx.surveyAnswer.create({
          data: {
            ...
            answerData: (a.answerData ?? a.value ?? {}) as Prisma.InputJsonValue,
            textContent:
              a.textContent != null
    ...
    let scoringResult: Awaited<
      ReturnType<ScoringService["gradeResponse"]>
    > = null;
        ...
        const row = await this.prisma.client.surveyResponse.update({
          ...
          data: {
            ...
            deviceInfo:
              ...
              undefined,
            source:
              body.source != null ? String(body.source) : existing.source,
            userId: userId != null ? BigInt(userId) : existing.userId,
          },
        });
        return {
