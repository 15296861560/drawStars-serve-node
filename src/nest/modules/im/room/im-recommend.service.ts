...
export class ImRecommendService {
  ...
  async recommend(
    ...
    const onlineList = await Promise.all(
      rooms.map(r => this.presence.getRoomOnlineCount(r.roomId))
    )
    ...
    const scored = rooms.map((r, i) => {
      ...
      const coldStart =
        Date.now() - Number(r.createdAt || 0) < 24 * 60 * 60 * 1000 ? 0.05 : 0
      ...
      const score =
        ...
        manual
      return {
        room: serializeBigInt({ ...r, onlineCount: online, msgCount1h: msg1h }),
        score
      }
    })
    ...
    return {
      list: scored.slice(start, start + pageSize).map(s => s.room),
      total: scored.length,
      curPage,
      pageSize
    }
  ...
  private async countMsgPerRoom(
    ...
    for (const r of rooms) {
      ...
      const c = await prisma.imMessage.count({
