...
export class ImMessageController {
  ...
  @Post(":msgId/forward")
  async forward(@Req() req: any, @Param("msgId") msgId: string, @Body() body: { target: { convType: string; bizId: string } }) {
    const uid = String(req.auth?.uid ?? "");
    const data = await this.messageService.forward(uid, msgId, body.target as never);
    return { status: true, msg: "ok", data: serializeBigInt(data) };
  }
}
