...
export class AnalyticsService {
  ...
  async updateWebsite(
    ...
    const domain =
      body.domain !== undefined ? body.domain?.trim() || null : cur.domain ?? null;
    await this.db().$executeRawUnsafe(
  ...
  async getStats(idOrUuid: string, query: Record<string, string>) {
    ...
    const periodStats = async (from: Date, to: Date) => {
      const [pvRows, uvRows, visitRows, bounceRows, timeRows] = await Promise.all([
    ...
    return {
      ...
      msg: "ok",
      data: {
        pageviews: { value: cur.pageviews, change: cur.pageviews - prev.pageviews },
        ...
        totaltime: { value: cur.totaltime, change: cur.totaltime - prev.totaltime },
      },
    };
  }
