...
export class StatisticsService {
  ...
  async questionStats(surveyId: number, questionId: number, auth?: AuthInfo) {
    await this.surveyService.assertSurveyAccess(surveyId, auth)
    ...
    if (
      ...
      const avg =
        nums.length > 0 ? nums.reduce((x, y) => x + y, 0) / nums.length : 0
    ...
    const lengths = samples.map(s => s.text.length).filter(n => n > 0)
  ...
  async crossAnalysis(
    ...
    const rows = [...rowKeys]
    const cols = [...colKeys]
    const cells = rows.map(rk => cols.map(ck => matrix.get(rk)?.get(ck) || 0))
  ...
  async qualityAnalysis(surveyId: number, auth?: AuthInfo) {
    ...
    const openQids = new Set(
      (survey?.questions || [])
        .filter(q => ['input', 'textarea', 'text'].includes(q.type))
        .map(q => Number(q.id))
    )
    ...
    for (const qid of openQids) {
      const texts = responses
        .map(r => {
          ...
          const text =
            ans?.textContent || String(asRecord(ans?.answerData).value ?? '')
  ...
  async markQuality(
    ...
    await this.prisma.client.surveyResponse.updateMany({
      ...
      data: {
        ...
        invalidReason: body.isValid ? null : body.reason || '人工标记无效'
      }
    })
  ...
  private buildFilterWhere(
    ...
    if (filter.isValid === true || filter.isValid === 'true')
      where.isValid = true
  ...
  async deleteReport(surveyId: number, reportId: number, auth?: AuthInfo) {
    await this.surveyService.assertSurveyAccess(surveyId, auth, {
      write: true
    })
  ...
  private async buildExportRows(
    ...
    const rows = responses.map(r => {
      const answerCells = questions.map(q => {
        const a = r.answers.find(x => Number(x.questionId) === Number(q.id))
        if (!a) return ''
  ...
  async getExportTask(surveyId: number, taskId: number, auth?: AuthInfo) {
    await this.surveyService.assertSurveyAccess(surveyId, auth)
