...
export class ImRelationService {
  ...
  async isBlocked(byUid: string, targetUid: string): Promise<boolean> {
    const row = await prisma.imBlacklist.findUnique({
      where: { userId_targetId: { userId: BigInt(byUid), targetId: BigInt(targetUid) } },
    });
    return !!row;
  }

  /** 是否允许陌生人私信：好友直接放行；否则按 §4.2.3 破冰 + 频控 */
  async assertStrangerCanSend(senderId: string, receiverId: string, convType: string, target?: { convType: string; bizId: string }) {
    if (convType !== "C2C") return; // 仅 C2C 受陌生人策略约束
    const isFriend = await this.isFriend(senderId, receiverId);
    if (isFriend) return;

    // 接收方是否允许陌生人私信
    const profile = await prisma.imUserProfile.findUnique({ where: { userId: BigInt(receiverId) } });
    ...
    // 陌生人频控：≤5 人/日，≤10 条/日
    const dk = dayKey();
    const usersCount = await incr(imRedisKey.strangerMsgUserDay(senderId, dk), 86400);
    const msgCount = await incr(imRedisKey.strangerMsgCountDay(senderId, dk), 86400);
    if (usersCount > IM_DEFAULTS.strangerPerDayUsers || msgCount > IM_DEFAULTS.strangerPerDayMsgs) {
      throw new ImException(ImErrorCode.STRANGER_LIMIT);
    }
  }

  /** 破冰条件：双方最近 24h 在同一房间有公开互动（简化为同房间成员且近24h有发言） */
  private async canIceBreak(senderId: string, receiverId: string, target?: { convType: string; bizId: string }): Promise<boolean> {
    ...
    if (conv) {
      const count = await prisma.imMessage.count({
        where: { conversationId: conv.id, senderId: BigInt(senderId), serverTime: { gt: since } },
      });
