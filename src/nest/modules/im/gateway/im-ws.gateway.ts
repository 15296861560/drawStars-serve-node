...
export class ImWsGateway implements OnModuleInit {
  ...
  private async handleConnection(ws: WebSocket, req: { url?: string }) {
    ...
    // 心跳
    conn.heartbeatTimer = setInterval(() => this.heartbeat(conn), IM_DEFAULTS.wsHeartbeatSec * 1000);

    ws.on("message", (raw) => this.handleMessage(conn, raw));
    ws.on("close", () => this.handleClose(conn));
    ws.on("error", () => this.handleClose(conn));

    ws.send(JSON.stringify({ event: WsEventType.PONG, ts: Date.now(), data: { connected: true } }));
  }

  private kickOldConns(uid: string, newConnId: string) {
    for (const [cid, c] of this.conns) {
      if (c.uid === uid && cid !== newConnId) {
        this.send(cid, { event: WsEventType.TAB_TAKEOVER, ts: Date.now(), data: { fromTabId: cid } });
        try {
  ...
  private async handleMessage(conn: InternalConn, raw: WebSocket.RawData) {
    ...
    switch (msg.type) {
      case WsEventType.PING:
        this.send(conn.connId, { event: WsEventType.PONG, ts: Date.now(), data: {} });
        break;
      ...
      case WsEventType.READ:
        if (msg.conversationId && msg.seq) {
          ...
          this.send(conn.connId, { event: WsEventType.ACK, requestId: msg.requestId, ts: Date.now(), data: { ok: true } });
        }
        break;
  ...
  private async handleRoomPresence(conn: InternalConn, roomId: string) {
    ...
    conn.presenceTimer = setTimeout(
      ...
      IM_DEFAULTS.roomPresenceSec * (IM_DEFAULTS.roomPresenceMissLimit + 1) * 1000,
    );
  }
  ...
  private async handleSend(conn: InternalConn, msg: WsIncoming) {
    ...
    try {
      ...
      this.send(conn.connId, {
        ...
        ts: Date.now(),
        data: { ok: true, msgId: result.msgId, seq: result.seq, conversationId: result.conversationId },
      });
    } catch (e) {
  ...
  private handlePush(msg: PushMessage) {
    ...
    for (const conn of this.conns.values()) {
      ...
      this.send(conn.connId, { event: WsEventType.MESSAGE, ts: Date.now(), data: { ...msg.envelope, conversationId: msg.conversationId, convType: msg.convType } });
    }
  }
