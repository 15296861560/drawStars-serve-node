...
export class ImAnalyticsService {
  ...
  async roomRanking(topN = 10) {
    ...
    const rooms = await prisma.imRoom.findMany({ where: { status: "ACTIVE", categoryId: { not: "CONFIDE" } } });
    ...
    for (const r of rooms) {
      ...
      const msgCount = conv ? await prisma.imMessage.count({ where: { conversationId: conv.id, serverTime: { gt: since } } }) : 0;
