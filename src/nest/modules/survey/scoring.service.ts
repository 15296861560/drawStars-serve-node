...
export class ScoringService {
  ...
  gradeQuestion(
    ...
    if (type === "radio" || type === "judge" || type === "select") {
      ...
      const actual = (
        Array.isArray(answerValue) ? answerValue : answerValue != null ? [answerValue] : []
      ).map(String);
  ...
  private mapGrade(
    ...
    const gradeRule = rules.find(
      (r) => asRecord(r).type === "grade_mapping",
    );
  ...
  private applyScoringRules(opts: {
    ...
    const detailByQ = new Map(
      opts.details.map((d) => [d.questionId, d] as const),
    );
  ...
  async submitGrade(
    ...
    const items = (Array.isArray(detail.items) ? detail.items : []).map(
      (it) => asRecord(it),
    );
  ...
  async reviewGrade(
    ...
    const result = await this.submitGrade(
      surveyId,
      body.responseId,
      auth,
      body,
