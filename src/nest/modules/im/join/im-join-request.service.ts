...
export class ImJoinRequestService {
  ...
  async list(uid: string, scope: 'mine' | 'manage') {
    ...
    // 我管理的（房主/管理员）
    const rooms = await prisma.imRoomMember.findMany({
      where: {
        userId: BigInt(uid),
        role: { in: [MemberRole.OWNER, MemberRole.ADMIN] }
      }
    })
    const groups = await prisma.imGroupMember.findMany({
      where: {
        userId: BigInt(uid),
        role: { in: [MemberRole.OWNER, MemberRole.ADMIN] }
      }
    ...
    const roomIds = await Promise.all(
      rooms.map(
        async r =>
          (await prisma.imRoom.findUnique({ where: { id: r.roomId } }))?.roomId
      )
    )
    const groupIds = await Promise.all(
      groups.map(
        async g =>
          (await prisma.imGroup.findUnique({ where: { id: g.groupId } }))
            ?.groupId
      )
    )
    const targets = [
      ...roomIds
        .filter(Boolean)
        .map(id => ({ targetType: TargetType.ROOM, targetId: id as string })),
      ...groupIds
        .filter(Boolean)
        .map(id => ({ targetType: TargetType.GROUP, targetId: id as string }))
    ]
    if (!targets.length) return []
    const orCond = targets.map(t => ({
      targetType: t.targetType,
      targetId: t.targetId
    }))
    const rows = await prisma.imJoinRequest.findMany({
      where: { OR: orCond, status: JoinStatus.PENDING },
      orderBy: { createdAt: 'desc' }
    })
    return serializeBigInt(rows)
  }
  ...
  async approve(uid: string, reqId: string) {
    ...
    await prisma.$transaction(async tx => {
      ...
      if (req.targetType === TargetType.ROOM) {
        const room = await tx.imRoom.findUnique({
          where: { roomId: req.targetId }
        })
        if (room) {
          await tx.imRoomMember.upsert({
            where: {
              roomId_userId: { roomId: room.id, userId: req.applicantId }
            },
            create: {
              roomId: room.id,
        ...
        const group = await tx.imGroup.findUnique({
          where: { groupId: req.targetId }
        })
        if (group) {
          await tx.imGroupMember.upsert({
            where: {
              groupId_userId: { groupId: group.id, userId: req.applicantId }
            },
            create: {
              groupId: group.id,
              userId: req.applicantId,
    ...
    const convId = await this.convService.upsertContainer(
      convType as never,
      req.targetId
    )
    await this.convService.ensureConvUser(convId, String(req.applicantId))
    await this.pushBus.publishEvent(
      String(req.applicantId),
      WsEventType.JOIN_REQUEST,
      {
        requestId: reqId,
  ...
  async reject(uid: string, reqId: string) {
    ...
    if (!req || req.status !== JoinStatus.PENDING)
      throw new ImException(ImErrorCode.NO_PERMISSION, '无待处理申请')
    await this.assertManager(uid, req.targetType, req.targetId)
    await prisma.imJoinRequest.update({
      where: { id: req.id },
      data: {
  ...
  async withdraw(uid: string, reqId: string) {
    ...
    if (!req || req.applicantId !== BigInt(uid))
      throw new ImException(ImErrorCode.NO_PERMISSION)
    if (req.status !== JoinStatus.PENDING)
      throw new ImException(ImErrorCode.NO_PERMISSION, '非待处理状态')
    await prisma.imJoinRequest.update({
      where: { id: req.id },
      data: { status: JoinStatus.CANCELLED, updatedAt: BigInt(Date.now()) }
    })
  ...
  private async assertManager(
    ...
    if (targetType === TargetType.ROOM) {
      ...
      const group = await prisma.imGroup.findUnique({
  ...
  private async notifyManagers(
    ...
    const conv = await prisma.imConversation.findUnique({
      where: { convType_bizId: { convType: convType, bizId: targetId } }
    })
    if (!conv) return
    const members =
      targetType === TargetType.ROOM
        ? await prisma.imRoomMember.findMany({
            where: {
              roomId: (await prisma.imRoom.findUnique({
                where: { roomId: targetId }
              }))!.id,
              role: { in: [MemberRole.OWNER, MemberRole.ADMIN] }
            }
          })
        : await prisma.imGroupMember.findMany({
            where: {
              groupId: (await prisma.imGroup.findUnique({
                where: { groupId: targetId }
              }))!.id,
              role: { in: [MemberRole.OWNER, MemberRole.ADMIN] }
            }
    ...
    for (const m of members) {
      ...
      await this.pushBus.publishEvent(
        String(m.userId),
        WsEventType.JOIN_REQUEST,
        { targetType, targetId, applicantId, status }
      )
    }
  }
}
