...
/** 从路径 / 文本推断操作类型 */
export function inferOperationType(method: string, pathOrText: string): OpType {
  ...
  if (/create|insert|add|register|\/save|新增|添加|注册/.test(hay)) {
...
export function normalizeOperationType(
  ...
  const rawOp =
    content.operation != null ? String(content.operation).trim() : ''
