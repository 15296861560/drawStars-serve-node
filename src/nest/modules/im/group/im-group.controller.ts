...
export class ImGroupController {
  ...
  @Post(":groupId")
  async update(@Req() req: any, @Param("groupId") groupId: string, @Body() body: Record<string, unknown>) {
    const uid = String(req.auth?.uid ?? "");
  ...
  @Post(":groupId/mute")
  async mute(@Req() req: any, @Param("groupId") groupId: string, @Body() body: { targetUserId: string; duration: "10min" | "1h" | "permanent" }) {
    const uid = String(req.auth?.uid ?? "");
    const data = await this.groupService.mute(groupId, uid, body.targetUserId, body.duration);
    return { status: true, msg: "ok", data: serializeBigInt(data) };
  }

  @Post(":groupId/kick")
  async kick(@Req() req: any, @Param("groupId") groupId: string, @Body() body: { targetUserId: string }) {
    const uid = String(req.auth?.uid ?? "");
  ...
  @Post(":groupId/admins")
  async setAdmin(@Req() req: any, @Param("groupId") groupId: string, @Body() body: { targetUserId: string; isAdmin: boolean }) {
    const uid = String(req.auth?.uid ?? "");
    const data = await this.groupService.setAdmin(groupId, uid, body.targetUserId, body.isAdmin);
    return { status: true, msg: "ok", data: serializeBigInt(data) };
  }

  @Post(":groupId/transfer")
  async transfer(@Req() req: any, @Param("groupId") groupId: string, @Body() body: { targetUserId: string }) {
    const uid = String(req.auth?.uid ?? "");
    const data = await this.groupService.transfer(groupId, uid, body.targetUserId);
    return { status: true, msg: "ok", data: serializeBigInt(data) };
  }
