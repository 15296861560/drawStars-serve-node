...
export class MobileService {
  ...
  async download(code: string, platform?: string, userId?: number) {
    ...
    const token = createHmac("sha256", process.env.MOBILE_DOWNLOAD_SECRET || "ds-mobile")
  ...
  async publishFromAppManage(appId: number) {
    ...
    if (needsOnlineUrl(platforms)) {
      ...
      if (!isHttpsUrl(app.moduleUrl) && !String(app.moduleUrl).startsWith("/")) {
        return {
          status: false,
    ...
    const moduleCode =
      app.moduleCode ||
      "app." + String(app.name || "mod").replace(/\s+/g, "_").toLowerCase();
  ...
  async setModulePermissions(moduleCode: string, permissionCodes: string[]) {
    ...
    return { status: true, msg: "ok", data: { moduleCode, permissionCodes: list } };
  }
