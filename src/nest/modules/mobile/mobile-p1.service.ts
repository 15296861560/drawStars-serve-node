...
export class MobileP1Service {
  ...
  async search(userId: number | undefined, q: string) {
    ...
    if (!keyword) {
      return { status: true, msg: "ok", data: { modules: [], menus: [], messages: [] } };
    }
    ...
    const modules: Array<{ moduleCode: string; name: string; version: string | null }> = [];
    for (const m of mods) {
    ...
    try {
      ...
      menus = (rows || [])
        .filter((r: any) => !r.client || r.client === "mobile" || r.client === "all")
        .map((r: any) => ({
  ...
  async submitFeedback(
    ...
    const dedupKey = String(userId || 0) + ":" + createHash("sha1").update(content).digest("hex");
  ...
  async createPreview(body: {
    ...
    if (!moduleUrl) return { status: false, msg: "MODULE_URL_REQUIRED", data: null };
  ...
  async checkDebugUrl(url?: string) {
    ...
    if (!u) return { status: false, msg: "URL_REQUIRED", data: { allow: false } };
  ...
  async setPushPrefs(
    ...
    if (userId == null) return { status: false, msg: "LOGIN_REQUIRED", data: null };
    const now = Date.now();
    try {
