/**
 * docxToHtml.js - 将Word文档(.docx)转换为HTML
 */
 ...
 * @returns {Promise<Object>} - 包含HTML内容和媒体文件的对象
 */
async function docxToHtml(docxFile) {
    try {
        // 处理输入，支持文件路径或Buffer
        let buffer;
        if (typeof docxFile === 'string') {
            const fileSize = fs.statSync(docxFile).size;
            
            // 对于大文件使用流式处理
            if (fileSize > 20 * 1024 * 1024) { // 超过20MB的文件
                return new Promise((resolve, reject) => {
                    const readStream = fs.createReadStream(docxFile);
                    const chunks = [];
                    ...
                    readStream.on('end', () => {
                        try {
                            ...
                            resolve(processDocx(fileBuffer));
                        } catch (error) {
                            reject(error);
                        }
                    });
...
/**
 * 处理docx缓冲区数据
 * @param {Buffer} buffer - docx文件的Buffer
 * @returns {Object} - 包含HTML内容和媒体文件的对象
 */
function processDocx(buffer) {
    // 解压docx文件（使用临时文件避免内存占用过高）
    const tempDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'docx-extract-'));
    const zip = new AdmZip(buffer);
    zip.extractAllTo(tempDir, true); // 直接提取到临时目录
    ...
    try {
        ...
        const relationshipsPath = path.join(tempDir, 'word', '_rels', 'document.xml.rels');
        
        // 读取XML文件（按需读取，避免一次性加载全部）
        ...
        if (fs.existsSync(numberingPath)) {
            const numberingContent = fs.readFileSync(numberingPath, 'utf8');
        ...
        if (fs.existsSync(relationshipsPath)) {
            const relationshipsContent = fs.readFileSync(relationshipsPath, 'utf8');
            try {
        ...
        for (const entry of mediaEntries) {
            ...
            mediaFiles[fileName] = path.join(tempDir, entry.entryName);
        }
        
        // 解析图表数据（保持原有逻辑，但优化内存使用）
        const chartData = {};
...
function parseDocumentToHtml(documentObj, stylesObj, numberingObj, relationships, mediaFiles, chartData, chartRelationships, zip) {
    try {
        ...
        <style>
            @font-face {
                font-family: 'SimSun-local';
                src: local('SimSun');
            }
            @font-face {
                font-family: 'SimHei-local';
                src: local('SimHei');
            }
            @font-face {
                font-family: 'KaiTi-local';
                src: local('KaiTi');
            }
            @font-face {
            ...
            .word-content h1 { font-size: 24pt; }
            .word-content h2 { font-size: 18pt; }
            .word-content h3 { font-size: 14pt; }
            .word-content h4 { font-size: 12pt; }
            .word-content h5 { font-size: 10pt; }
            .word-content h6 { font-size: 8pt; }
            .word-content table {
            ...
            .word-content table.floating-table {
                position: relative;
                z-index: 1;
                margin: 0;
                box-sizing: border-box;
            }
            .word-content .table-container {
                margin: 8pt 0;
                overflow: hidden;
                position: relative;
            }
