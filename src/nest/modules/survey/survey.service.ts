...
export class SurveyService {
    ...
    return {
      ...
      publishConfig: survey.publishConfig
        ? {
            ...
            publishTime: survey.publishConfig.publishTime?.toISOString() ?? null,
  ...
  async list(
    ...
    if (query.mine === "1" || query.mine === "true" || !this.hasListPermission(auth)) {
  ...
  async create(auth: AuthInfo | undefined, body: Record<string, unknown>) {
    ...
    const survey = await this.prisma.client.survey.create({
      data: {
        ...
        scoringConfig: (body.scoringConfig as Prisma.InputJsonValue) ?? undefined,
    ...
    const created = await this.prisma.client.$transaction(async (tx) => {
      const survey = await tx.survey.create({
        data: {
          ...
          scoringConfig: (src.scoringConfig as Prisma.InputJsonValue) ?? undefined,
    ...
    await this.prisma.client.$transaction(async (tx) => {
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        const fields = {
          ...
          description:
            q.description != null ? String(q.description) : null,
  ...
  async sortQuestions(
    id: number,
    auth: AuthInfo | undefined,
    body: { orders?: Array<{ id: number; sortOrder: number; pageIndex?: number }> },
  ) {
    await this.assertSurveyAccess(id, auth, { write: true });
          ...
          data: {
            ...
            targetQuestionId:
              r.targetQuestionId != null
                ...
                : null,
            targetPage:
              r.targetPage != null ? Number(r.targetPage) : null,
