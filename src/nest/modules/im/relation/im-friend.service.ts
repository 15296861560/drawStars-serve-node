...
export class ImFriendService {
  ...
  async request(uid: string, friendId: string, remark?: string) {
    if (uid === friendId) throw new ImException(ImErrorCode.NO_PERMISSION, "不能添加自己");
    const exist = await prisma.imFriendship.findUnique({
      where: { userId_friendId: { userId: BigInt(uid), friendId: BigInt(friendId) } },
    });
    if (exist) {
      if (exist.status === FriendStatus.ACCEPTED) throw new ImException(ImErrorCode.NO_PERMISSION, "已是好友");
      if (exist.status === FriendStatus.PENDING) throw new ImException(ImErrorCode.JOIN_PENDING, "已申请，等待对方同意");
      if (exist.status === FriendStatus.REJECTED) {
        const cooldown = IM_DEFAULTS.friendRejectCooldownSec * 1000;
        if (exist.updatedAt && Number(exist.updatedAt) + cooldown > Date.now()) {
          throw new ImException(ImErrorCode.FREQ_LIMIT, "对方拒绝后 7 日内不可重复申请");
        }
        // 冷却结束，重新进入 PENDING
        const updated = await prisma.imFriendship.update({
          where: { id: exist.id },
          data: { status: FriendStatus.PENDING, remark, updatedAt: BigInt(Date.now()) },
        });
    ...
    const created = await prisma.imFriendship.create({
      data: { userId: BigInt(uid), friendId: BigInt(friendId), status: FriendStatus.PENDING, remark, createdAt: BigInt(Date.now()), updatedAt: BigInt(Date.now()) },
    });
    await this.notifyFriendRequest(uid, friendId, "PENDING");
  ...
  async respond(uid: string, friendId: string, accept: boolean) {
    const req = await prisma.imFriendship.findUnique({
      where: { userId_friendId: { userId: BigInt(friendId), friendId: BigInt(uid) } },
    });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (accept) {
        ...
        if (reverse) {
          await tx.imFriendship.update({ where: { id: reverse.id }, data: { status: FriendStatus.ACCEPTED, updatedAt: BigInt(Date.now()) } });
        } else {
          await tx.imFriendship.create({
            data: { userId: BigInt(uid), friendId: BigInt(friendId), status: FriendStatus.ACCEPTED, createdAt: BigInt(Date.now()), updatedAt: BigInt(Date.now()) },
          });
        }
      }
    });
    await this.notifyFriendResult(uid, friendId, accept ? "ACCEPTED" : "REJECTED");
  ...
  private async notifyFriendRequest(fromUid: string, toUid: string, status: string) {
    await this.pushBus.publishEvent(toUid, WsEventType.FRIEND_REQUEST, { fromUid, status });
  }

  private async notifyFriendResult(toUid: string, fromUid: string, status: string) {
    await this.pushBus.publishEvent(toUid, WsEventType.FRIEND_RESULT, { fromUid, status });
  }
}
