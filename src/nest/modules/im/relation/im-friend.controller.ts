...
export class ImFriendController {
  ...
  @Post("request")
  async request(@Req() req: any, @Body() body: { friendId: string; remark?: string }) {
    const uid = String(req.auth?.uid ?? "");
    const data = await this.friendService.request(uid, body.friendId, body.remark);
    return { status: true, msg: "ok", data: serializeBigInt(data) };
  }

  @Post("respond")
  async respond(@Req() req: any, @Body() body: { friendId: string; accept: boolean }) {
    const uid = String(req.auth?.uid ?? "");
    const data = await this.friendService.respond(uid, body.friendId, body.accept);
    return { status: true, msg: "ok", data: serializeBigInt(data) };
  }
