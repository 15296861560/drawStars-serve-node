...
export class ImMessageService {
  ...
  async send(senderId: string, dto: SendMessageDto): Promise<SendResult> {
    if (!dto.clientMsgId)
      throw new ImException(ImErrorCode.NO_PERMISSION, 'clientMsgId 必填')
    if (!dto.msgType)
      throw new ImException(ImErrorCode.NO_PERMISSION, 'msgType 必填')

    // 1. 解析/创建会话 + 校验发送者权限
    const { convId, convType, bizId, receiverId } =
      await this.resolveConversation(senderId, dto)
    ...
    const targetUids = await this.resolvePushTargets(
      convId,
      convType,
      bizId,
      senderId
  ...
  async recall(operatorId: string, msgId: string) {
    ...
    if (msg.status === MsgStatus.RECALLED)
      throw new ImException(ImErrorCode.NO_PERMISSION, '已撤回')
    ...
    await this.rollbackUnreadOnRecall(
      msg.conversationId,
      String(msg.senderId),
      Number(msg.seq)
    )
  ...
  async forward(
    operatorId: string,
    msgId: string,
    target: { convType: ConvType; bizId: string }
  ) {
    const msg = await prisma.imMessage.findUnique({ where: { msgId } })
    if (!msg) throw new ImException(ImErrorCode.NO_PERMISSION, '消息不存在')
    // 源会话若为倾诉房禁转
    const sourceConv = await prisma.imConversation.findUnique({
      where: { id: msg.conversationId }
    })
    if (sourceConv?.convType === ConvType.ROOM) {
      const room = await prisma.imRoom.findUnique({
        where: { roomId: sourceConv.bizId }
      })
  ...
  private async resolveConversation(
    ...
    if (dto.conversationId) {
      const conv = await prisma.imConversation.findUnique({
        where: { id: BigInt(dto.conversationId) }
      })
      if (!conv) throw new ImException(ImErrorCode.NO_PERMISSION, '会话不存在')
      await this.convService.assertMember(conv.id, senderId)
      const receiverId =
        conv.convType === ConvType.C2C && conv.pairKey
          ? String(conv.pairKey)
              .split('_')
              .find(x => x !== String(senderId))
          : undefined
      return {
        convId: conv.id,
        convType: conv.convType as ConvType,
        bizId: conv.bizId,
        receiverId
      }
    }
    if (!dto.target)
      throw new ImException(ImErrorCode.NO_PERMISSION, 'target 必填')
  ...
  private async assertInContainer(
    convType: ConvType,
    bizId: string,
    uid: string
  ) {
    if (convType === ConvType.GROUP) {
      const m = await prisma.imGroupMember.findUnique({
        where: {
          groupId_userId: { groupId: BigInt(bizId), userId: BigInt(uid) }
        }
      })
  ...
  private async getMemberRole(
    conv: { convType: string; bizId: string },
    uid: string
  ): Promise<string | null> {
    if (conv.convType === ConvType.GROUP) {
      const m = await prisma.imGroupMember.findUnique({
        where: {
          groupId_userId: { groupId: BigInt(conv.bizId), userId: BigInt(uid) }
        }
      })
      return m?.role ?? null
    }
  ...
  private async preSendChecks(
    ...
    const secCount = await incr(
      imRedisKey.freqUserConv(senderId, String(convId)),
      1
    )
    if (secCount > IM_DEFAULTS.freqPerSec)
      throw new ImException(ImErrorCode.FREQ_LIMIT)
    ...
    if (convType === ConvType.C2C && receiverId) {
      const blockedByReceiver = await this.relation.isBlocked(
        receiverId,
        senderId
      )
      if (blockedByReceiver) throw new ImException(ImErrorCode.BLOCKED)
      await this.relation.assertStrangerCanSend(
        senderId,
        receiverId,
        convType,
        dto.target
  ...
  private async assertNotMuted(convType: ConvType, bizId: string, uid: string) {
    ...
    if (convType === ConvType.GROUP) {
      ...
      if (speakMode === 'MUTE_ALL' && role === 'MEMBER')
        throw new ImException(ImErrorCode.MUTED)
      if (speakMode === 'ADMIN_ONLY' && role === 'MEMBER')
        throw new ImException(ImErrorCode.MUTED)
    }
  ...
  private async updateUnread(
    ...
    if (convType === ConvType.C2C) {
      ...
      if (other) {
        await tx.imConversationUser.updateMany({
          ...
          data: {
            ...
            mentionUnread:
              atUserIds.includes(other) || atAll ? { increment: 1 } : undefined,
            updatedAt: BigInt(Date.now())
          }
        })
      }
    } else if (convType === ConvType.GROUP) {
      const members = await tx.imGroupMember.findMany({
        where: { groupId: BigInt(bizId) }
      })
      for (const m of members) {
        ...
        await tx.imConversationUser.updateMany({
          ...
          data: {
            unreadCount: { increment: 1 },
            mentionUnread:
              atUserIds.includes(String(m.userId)) || atAll
                ? { increment: 1 }
                : undefined,
            updatedAt: BigInt(Date.now())
          }
      ...
      const members = await tx.imRoomMember.findMany({
        where: {
          roomId: (await tx.imRoom.findUnique({ where: { roomId: bizId } }))!.id
        }
      })
      for (const m of members) {
        if (String(m.userId) === senderId) continue
  ...
  private async rollbackUnreadOnRecall(
    convId: bigint,
    senderId: string,
    seq: number
  ) {
