...
export class ImBlacklistController {
  ...
  async list(@Req() req: any) {
    ...
    const rows = await prisma.imBlacklist.findMany({ where: { userId: uid }, orderBy: { createdAt: "desc" } });
  ...
  async remove(@Req() req: any, @Body() body: { targetId: string }) {
    const uid = BigInt(req.auth?.uid ?? 0);
    const target = BigInt(body.targetId);
    await prisma.imBlacklist.deleteMany({ where: { userId: uid, targetId: target } });
